...
리피터 허브나 스위칭 허브에서 경유한 패킷은 결국 라우터에 도착한다

스위칭 허브와의 공통점은 중계 대상을 등록한 표를 보고 패킷을 어디로 중계할지 판단하는 부분

구체적으론 다름. 라우터의 바탕이 되는 IP라는 개념이 스위칭 허브의 바탕이 되는 이더넷과 다르다

라우터 내부 구조는 중계 부분과 포트 부분이라는 두 부분으로 구성된다

- 중계 부분 : 패킷의 중계 대상을 판단하는 동작 담당
- 포트 부분 : 패킷을 송, 수신하는 동작 담당

이런 역할 분담은 프로토콜 스택의 IP 담당 부분과 LAN 어댑터의 역할 분담과 같다.
...
`스위칭 허브`는 MAC 헤더에 기록되어 있는 수신처 MAC 주소로 중계 대상 판단

`라우터`는 IP 헤더에 기재되어 있는 수신처 IP 주소로 중계 대상 판단
...
> 
- 주소 집약이라는 개념을 이용하여 몇 개의 서브넷을 모아 한 개의 서브넷으로 간주한 후 묶은 서브넷을 경로표에 등록할 수 있다
- 반대로 한 개의 서브넷을 세분화하여 경로표에 등록하고 복수의 서브넷이 있는 것 처럼 보일 수도 있다

> 라우터의 경로표에서 ‘넷마스크’ 항목은 경로표의 수신처와 패킷의 수신처 주소를 대조할 때 비트 수를 나타낸다
> 

스위칭 허브는 MAC 주소 테이블에 정보를 등록하는 동작 실행함. 패킷을 중계할 때 경로표의 내용에 손대지 않는다

라우터가 경로표에 경로 정보를 등록하거나 갱신하는 동작은 패킷을 중계하는 동작과 분리되어 있다