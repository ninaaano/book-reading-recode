...
- 타임아웃 값 : ACK번호가 돌아오는 것을 기다리는 시간, 즉 대기 시간

네트워크가 정체가 일어나면 ACK 번호가 돌아오는 것이 지연될 수 있으므로 이것을 예측해서 대기 시간을 어느정도 길게 설정해야 한다. 그렇지 않으면 ACK 번호가 돌아오기 전에 다시 보내야할 수 있다

그렇다고 대기 시간이 너무 길어지면 패킷을 다시 보내는 동작이 지연되어 속도 저하의 원인이 된다

TCP는 대기 시간을 동적으로 변경하는 방법을 선택하고 있다. 데이터 송신 동작을 실행하고 있을 때 항상 ACK 번호가 돌아오는 시간을 계측하고 지연되면 대기 시간을 늘린다. ACK 번호가 곧바로 돌아오면 대기 시간을 짧게 설정한다.

> OS에 따라 다르지만 최소값은 0.5초에서 1초 정도로 설정되어 있다
>

## 윈도우 제어 방식으로 효율적으로 ACK 번호를 관리한다

ACK 번호가 돌아올 때 까지 기다리는 것은 시간 낭비이므로 이것을 줄이기 위해 윈도우 제어 방식에 따라 송신과 ACK 번호 통지의 동작을 실행한다.

- 윈도우 제어는 한 개의 패킷을 보낸 후 ACK 번호를 기다리지 않고 차례대로 연속해서 복수의 패킷을 보내는 방법

ACK 번호를 기다리지 않고 차례로 패킷을 보내면 수신측의 능력을 초과하여 패킷을 보내는 사태가 일어날 수 있다

- 수신 측에서 송신 측에 수신 가능한 데이터 양을 통지하고 수신 측은 이 양을 초과하지 않도록 송신 동작을 실행한다 → 윈도우 제어 방식

수신 측은 수신 버퍼에 데이터를 임시 보관하고 수신 처리를 진행한다. 수신 처리가 끝나고 버퍼에 빈 부분이 생기면 그 분량만큼 수신할 수 있는 데이터 양을 늘리므로 TCP 헤더의 윈도우 필드에서 이것을 송신측에 알린다. 이렇게 수신측의 능력을 초과해서 데이터를 보내는 일이 없어진다

- 윈도우 사이즈 : 수신 가능한 데이터 양의 최대값 → TCP를 정밀 조정하는 매개변수


## ACK 번호와 윈도우를 합승한다

- 윈도우 통지는 수신 측이 수신 버퍼에서 데이터를 추출하여 애플리케이션에 건네주었을 때 필요하다
    - 수신 측에서 애플리케이션에 데이터를 건내주고 수신 버퍼의 빈 영역이 늘어 났을 때 송신 측에 통지해야 할때가 윈도우 통지의 타이밍이다
- ACK 번호는 수신측에서 데이터를 받았을 때 내용을 조사하여 정산 수신을 확인할 수 있는 경우에만 송신 측에 보낸다
    - 데이터를 수신한 후 즉시 보낸다

⇒ 송신 측에서 보낸 데이터가 수신 측에 도착하여 수신 동작이 완료되었을 때 ACK 번호를 송신 측에 통지하고 데이터를 애플리케이션에 건네주었을 때 윈도우를 송신 측에 통지하는 상태

⇒ 수신 측에서 송신 측에 보내는 패킷이 많이져서 효율성이 저하된다

- 수신측은 소켓을 바로 보내지 않고 잠시 대기한다. 그 사이 다음 통지 동작이 일어나면 양쪽을 상승시켜 한 개의 패킷으로 묶어보낸다
    - ACK 번호의 송신을 대조할 때 윈도우 통지가 일어나면 ACK 번호와 윈도우를 한개의 패킷에 합승시켜서 통지하여 패킷의 수를 줄일 수 있다

## HTTP 응답 메세지를 수신한다

- 수신한 데이터 조각과 TCP 헤더의 내용을 조사하여 중간에 데이터가 누락되었는지 검사함
    - 문제 없으면 ACK 번호 반송
- 데이터 조각을 수신 버퍼에 일시 보관하고 조각을 연결하여 데이터를 원래 모습으로 복원한 후 애플리케이션에 건네준다
    - 수신 데이터를 애플리케이션이 지정한 메모리 영역에 옮겨 기록한 후 애플리케이션에 제어를 돌려준다는 말
- 애플리케이션에 데이터를 건네주고 타이밍을 가늠하여 윈도우를 송신측에 통지한다
...
데이터 송,수신 동작이 끝난 후 프로토콜 스택의 움직임은 어떻게 될까?
...
데이터 송,수신에 관한 일련의 움직임을 알아보자

- 데이터 송,수신을 종료하는 것은 애플리케이션이 송신해야 하는 데이터를 전부 송신 완료 했다고 판단 했을 때
- 송신을 완료한 측이 연결을 끊음
- 웹은 서버가 응답 메세지를 반송 완료하면 서버 측이 연결을 끊음
- 클라이언트 측이 데이터 보내기를 완료했다고 하는 식의 애플리케이션도 있겠지만 클라이언트 측에서 먼저 연결 끊기 단계에 들어간다는 말이 뭔 말장난 같은 소리임??
- 프로토콜 스택은 어느쪽에서 먼저 연결 끊기 단계에 들어가도 좋게 만들어짐
- 아무튼 데이터 보내기를 완료한 쪽이 연결 끊기 단계에 들어간다

**서버 측에서 연결을 끊었다?**

- 서버 측의 애플리케이션이 Socket 라이브러리의 close 호출
- 서버 측의 프로토콜 스택이 TCP 헤더를 만들고 연결 끊기를 나타내는 정보를 설정함
    - 컨트롤 비트의 FIN 비트에 1 설정, IP 담당 부분에 클라이언트에 송신하라고 의뢰
- 동시에 서버측의 소켓에 연결 끊기 동작에 들어갔다는 정보 기록

**클라이언트 측에서 연결을 끊었다?**

- 서버에서 FIN에 1을 설정한 TCP 헤더가 도착했다
- 클라이언트 측 프로토콜 스택은 자신의 소켓에 서버측이 연결 끊기 동작에 들어갔다는 것을 기록함
- FIN을 1로 설정한 패킷을 받은 사실을 알리기 위해 ACK 번호를 서버측에 반송
- 끝나면 애플리케이션이 데이터를 가지러 올 때까지 기다림

- 애플리케이션이 read를 호출하여 데이터를 가지러 옴
- 데이터를 안주고 서버에서 보낸 데이터를 전부 수신 완료했다는 사실을 클라이언트 측의 애플리케이션(브라우저)에 알림
- 웹의 동작은 서버가 응답을 반송하면 끝나도록 규칙이 정해져있음
- 서버에서 보낸 데이터를 전부 수신 완료하면 클라이언트도 종료

## 소켓을 말소한다

소켓은 바로 말소되지 않고 잠시 기다렸다가 말소됨

- 클라이언트에서 종료하여 클라이언트가 먼저 FIN을 보내고 마지막에 서버에 입장에서 ACK 번호가 돌아오지 않았을 때 새 소켓에 같은 포트가 할당 될 수 있기 때문
- 패킷이 없어 졌을 때 다시 보내는 동작은 몇 분 쯤 지속됨
- 시간이 경과하면 없다고 판단하고 다시 보내기 동작을 멈춤